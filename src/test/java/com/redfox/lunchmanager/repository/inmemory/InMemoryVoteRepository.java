...
public class InMemoryVoteRepository implements VoteRepository {
    ...
    {
        save(new Vote(user1, restaurant1, now()), user1.getId());
        save(new Vote(user2, restaurant4, now()), user2.getId());
        save(new Vote(user3, restaurant3, now()), user3.getId());
    }

    @Override
    public Vote save(Vote vote, int userId) {
        Objects.requireNonNull(vote, "vote must not be null");
        var votes = usersVotes.computeIfAbsent(userId, uid -> new InMemoryBaseRepository<>());
        return votes.save(vote);
    }

    @Override
    public boolean delete(int id, int userId) {
        var votes = usersVotes.get(userId);
